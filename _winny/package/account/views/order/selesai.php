			...
			<div class="card-body">
				<div class="row m-lr-0">
					...
					<div class="col-md-4 f-sm-12 f-md-16">
						...
						<span class="text-dark p-r-8 font-medium"><?php echo $this->setting->ubahTgl("d M Y H:i", $rx->selesai); ?> WIB</span>
					</div>
				</div>
			</div>
			<div class="card-footer">
				<div class="row">
					<div class="col-md-3">
						...
						$this->db->select("id");
						$this->db->where("idtransaksi", $rx->id);
						$ul = $this->db->get("sales_review");
						if ($ul->num_rows() > 0) {
						?>
							<a href="<?php echo site_url("account/order/ulasan/" . $rx->orderid); ?>" class="btn btn-primary btn-block m-b-10 f-sm-12 f-md-14">
								Lihat Ulasan
							</a>
						<?php
						} else {
					...
					<div class="col-md-4 text-right">
						<h5 class="text-dark f-sm-12 f-md-14">Total Order &nbsp;<strong><span class="color1 font-bold text-right">IDR <?php echo $this->setting->formUang($rx->ongkir + $totalproduk); ?></span></strong></h5>
					</div>
				</div>
			</div>
		</div>
