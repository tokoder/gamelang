<div class="row">
	<div class="col-xs-12 col-sm-6 col-sm-push-3 col-md-4 col-md-push-4">
		<h1 class="text-center"><?php _e('lang_create_account') ?></h1>
		<div class="panel panel-default">
			<div class="panel-body">
				<?php echo form_open('register', 'role="form" id="register"'), form_nonce('user-register'); ?>
					<div class="form-group<?php echo form_error('first_name') ? ' has-error' : '' ?>">
						<label for="first_name" class="sr-only"><?php _e('lang_first_name') ?></label>
					...
					<div class="form-group<?php echo form_error('last_name') ? ' has-error' : '' ?>">
						<label for="last_name" class="sr-only"><?php _e('lang_last_name') ?></label>
					...
					<div class="form-group<?php echo form_error('email') ? ' has-error' : '' ?>">
						<label for="email" class="sr-only"><?php _e('lang_email_ADDRESS') ?></label>
						<?php echo print_input($email, array('class' => 'form-control')) ?>
						<?php echo form_error('email', '<small class="help-block">', '</small>') ?>
					</div>
					<div class="form-group<?php echo form_error('username') ? ' has-error' : '' ?>">
						<label for="username" class="sr-only"><?php _e('lang_USERNAME') ?></label>
					...
					<div class="form-group<?php echo form_error('gender') ? ' has-error' : '' ?>">
						<label for="male" class="radio-inline"><input type="radio" name="gender" id="male" value="male" <?php echo set_radio('gender', 'male') ?>> <?php _e('lang_male') ?></label>
						<label for="female" class="radio-inline"><input type="radio" name="gender" id="female" value="female" <?php echo set_radio('gender', 'female') ?>> <?php _e('lang_female') ?></label>
				...
				<?php endif; ?>
					<button type="submit" class="btn btn-primary pull-right"><?php _e('lang_create_account') ?></button>
