...
global $cg_filter, $cg_actions, $cg_current_filter;
 ...
 */
$cg_filter = ( $cg_filter )
    ? Hooks::build_preinitialized_hooks( $cg_filter )
    : array();

// We make sure actions and current filter are always array.
isset($cg_actions) OR $cg_actions = array();
isset($cg_current_filter) OR $cg_current_filter = array();
...
if ( ! function_exists('add_filter'))
{
     ...
     *
     * @global Hooks[] $cg_filter A multidimensional array of all hooks and the callbacks hooked to them.
     *
     * @param string   $hook_name     The name of the filter to add the callback to.
     * @param callable $callback      The callback to be run when the filter is applied.
    ...
    function add_filter( $hook_name, $callback, $priority = 10, $accepted_args = 1 ) {
        ...
        if ( ! isset( $cg_filter[ $hook_name ] ) ) {
            $cg_filter[ $hook_name ] = new Hooks();
        }

        $cg_filter[ $hook_name ]->add_filter( $hook_name, $callback, $priority, $accepted_args );
...
if ( ! function_exists('apply_filters'))
{
     ...
     * Calls the callback functions that have been added to a filter hook.
     *
     * @global Hooks[] $cg_filter         Stores all of the filters and actions.
     * @global string[]  $cg_current_filter Stores the list of current filters with the current one last.
     *
    ...
    function apply_filters( $hook_name, $value, ...$args ) {
        global $cg_filter, $cg_current_filter;

        // Do 'all' actions first.
        if ( isset( $cg_filter['all'] ) ) {
            $cg_current_filter[] = $hook_name;
        ...
        $filtered = $cg_filter[ $hook_name ]->apply_filters( $value, $args );
...
if ( ! function_exists('apply_filters_ref_array'))
{
    ...
    function apply_filters_ref_array( $hook_name, $args ) {
        ...
        $filtered = $cg_filter[ $hook_name ]->apply_filters( $args[0], $args );
...
if ( ! function_exists('has_filter'))
{
    ...
    function has_filter( $hook_name, $callback = false ) {
        ...
        return $cg_filter[ $hook_name ]->has_filter( $hook_name, $callback );
    }
}
...
if ( ! function_exists('remove_filter'))
{
    ...
    function remove_filter( $hook_name, $callback, $priority = 10 ) {
        ...
        if ( isset( $cg_filter[ $hook_name ] ) ) {
            $r = $cg_filter[ $hook_name ]->remove_filter( $hook_name, $callback, $priority );

            if ( ! $cg_filter[ $hook_name ]->callbacks ) {
                unset( $cg_filter[ $hook_name ] );
            }
        }
...
if ( ! function_exists('remove_all_filters'))
{
     ...
     * @return true Always returns true.
     */
    function remove_all_filters( $hook_name = null, $priority = false ) {
        global $cg_filter;

        if ( isset( $cg_filter[ $hook_name ] ) ) {
            $cg_filter[ $hook_name ]->remove_all_filters( $priority );

            if ( ! $cg_filter[ $hook_name ]->has_filters() ) {
                unset( $cg_filter[ $hook_name ] );
            }
        }
...
if ( ! function_exists('current_filter'))
{
    ...
    function current_filter() {
        global $cg_current_filter;

        return end( $cg_current_filter );
    }
}
...
if ( ! function_exists('doing_filter'))
{
     ...
     * @see current_filter()
     * @see did_action()
     * @global string[] $cg_current_filter Current filter.
     *
    ...
    function doing_filter( $hook_name = null ) {
        ...
        if ( null === $hook_name ) {
            return ! empty( $cg_current_filter );
        }

        return in_array( $hook_name, $cg_current_filter, true );
    }
}
...
if ( ! function_exists('do_action'))
{
     ...
     *
     * @global Hooks[] $cg_filter         Stores all of the filters and actions.
     * @global int[]     $cg_actions        Stores the number of times each action was triggered.
     * @global string[]  $cg_current_filter Stores the list of current filters with the current one last.
     *
    ...
    function do_action( $hook_name, ...$arg ) {
        ...
        if ( ! isset( $cg_actions[ $hook_name ] ) ) {
            $cg_actions[ $hook_name ] = 1;
        } else {
            ++$cg_actions[ $hook_name ];
        }
        ...
        $cg_filter[ $hook_name ]->do_action( $arg );
...
if ( ! function_exists('do_action_ref_array'))
{
    ...
    function do_action_ref_array( $hook_name, $args ) {
        ...
        $cg_filter[ $hook_name ]->do_action( $args );
...
if ( ! function_exists('_call_all_hook'))
{
	 ...
	 * @access private
	 *
	 * @global WP_Hook[] $cg_filter Stores all of the filters and actions.
	 *
	...
	function _call_all_hook( $args ) {
		...
		$cg_filter['all']->do_all_hook( $args );
	}
}
// -----------------------------------------------------------------------------
